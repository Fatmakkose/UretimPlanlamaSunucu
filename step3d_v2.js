...
// Let's add the orderId variable at the top as well since it might be needed.
combinedJS = 'const orderId = @Model.Id;\n' + combinedJS;
...
const docReadyIndex = planHtml.indexOf("document.addEventListener('DOMContentLoaded', function() {");
if (docReadyIndex !== -1) {
    ...
    planHtml = planHtml.replace("document.addEventListener('DOMContentLoaded', function() {", "document.addEventListener('DOMContentLoaded', function() {\n        planRenderCuttingTable();");
    
    fs.writeFileSync(planPath, planHtml, 'utf8');
} else {
    console.log("Could not find document.addEventListener");
}
