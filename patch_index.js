...
// Replace HTML
const oldHtmlRegex = /<!-- Edit Model Detail Modal -->[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/div>/;
const newHtml = `<!-- Edit Model Detail Modal -->
...
content = content.replace("$(document).ready(function() {", jsInjection);

const editPopulation = `$("#editProductionJson").val(order.productionJson || "");
...
content = content.replace('$("#editProductionJson").val(order.productionJson || "");', editPopulation);

const submitOldRegex = /var acc = \{\s*KetenKarti: \$\("#editAcc_KetenKarti"\)\.is\(":checked"\)[\s\S]*?Diger: parseFloat\(\$\("#editAcc_Diger"\)\.val\(\)\) \|\| null\s*\};/;
content = content.replace(submitOldRegex, 'updateEditStockMaterialsJson();');

const formDataOld = 'var formData = $(this).serialize();';
const formDataNew = `var formData = $(this).serialize();
                    formData += "&OrderMaterialsJson=" + encodeURIComponent($('#editOrderMaterialsJson').val() || "[]");`;
content = content.replace(formDataOld, formDataNew);
